...
class DespesaDAO extends Conexao
{
    ...
    public function listarPorPessoa($pessoaId)
    {
        $sql = "SELECT t.*, c.nome as categoria_nome
                ...
                WHERE t.pessoa_id = ? AND t.tipo = 'Despesa' AND t.deletado_em IS NULL
    ...
    public function inserir($pessoaId, $descricao, $valor, $data, $categoriaId, $status = "Pendente")
    {
    ...
    {
        $sql = "SELECT * FROM transacoes WHERE id_transacao = ? AND tipo = 'Despesa' AND deletado_em IS NULL";
        try {
            $stm = $this->db->prepare($sql);
            $stm->bindValue(1, $id);
            $stm->execute();
    ...
    public function atualizar($id, $descricao, $valor, $data, $categoriaId, $status)
    {
        $sql = "UPDATE transacoes SET descricao = ?, valor = ?, data_movimentacao = ?,
                categoria_id = ?, status_transacao = ? WHERE id_transacao = ? AND tipo = 'Despesa'";
        try {
            $stm = $this->db->prepare($sql);
    ...
    {
        $sql = "UPDATE transacoes SET deletado_em = NOW() WHERE id_transacao = ? AND tipo = 'Despesa'";
        try {
            $stm = $this->db->prepare($sql);
            $stm->bindValue(1, $id);
            $stm->execute();
    ...
    public function totalDoMes($pessoaId)
    {
        $sql = "SELECT COALESCE(SUM(valor), 0) as total
                FROM transacoes
                WHERE pessoa_id = ? AND tipo = 'Despesa'
